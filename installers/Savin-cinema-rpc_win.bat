...
echo [1/8] 🔍 Buscando tu entorno de Python real...
...
:: Extraer la ruta real del binario para evitar fallos de alias con pythonw
for /f "delims=" %%I in ('python -c "import sys; print(sys.executable)"') do set "REAL_PYTHON=%%I"
set "REAL_PYTHONW=!REAL_PYTHON:python.exe=pythonw.exe!"
echo ✅ Python detectado en: !REAL_PYTHON!
echo ✅ Pythonw configurado en: !REAL_PYTHONW!
echo.
...
echo [4/8] 💻 Configuración del botón de la Landing Page...
echo ¿Deseas mostrar el botón personalizado hacia tu Web Oficial?
echo Presiona [ENTER] para Sí (Por defecto) o introduce n para No:
set /p "USER_INPUT_GH=❯ "
if /i "!USER_INPUT_GH!"=="n" set "FINAL_INCLUDE_GH=False"
echo ➡ Botón Web: !FINAL_INCLUDE_GH!
echo.
...
:: 8. Generar el disparador .lua asíncrono para Windows (Inyectando ruta absoluta de pythonw)
echo [8/8] 🌙 Sincronizando disparador Lua para Windows...
set "LUA_PYTHONW=!REAL_PYTHONW:\=\\!"
(
...
echo     args = {"!LUA_PYTHONW!", mp.command_native^("expand-path", "~~/savin_cinema_rpc.py"^)},
...
##PY:GITHUB_URL = 'https://mvp-savyn.github.io/Savin-cinemaRPC/'
...
##PY:    
##PY:    tiene_numeros = bool(re.search(r'\d+', clean_title))
##PY:    if tiene_numeros:
##PY:        orden_categorias = ["tv", "movie"]
##PY:    else:
##PY:        orden_categorias = [media_type, "tv" if media_type == "movie" else "movie"]
##PY:        
##PY:    estrategias_busqueda = []
##PY:    estrategias_busqueda.append(clean_title)
##PY:    
##PY:    titulo_sin_numeros = re.sub(r'\d+', ' ', clean_title)
##PY:    titulo_sin_numeros = re.sub(r'\s+', ' ', titulo_sin_numeros).strip('. -_ ')
##PY:    if titulo_sin_numeros and titulo_sin_numeros != clean_title:
##PY:        estrategias_busqueda.append(titulo_sin_numeros)
##PY:        
##PY:    for titulo_query in estrategias_busqueda:
##PY:        for m_type in orden_categorias:
##PY:            search_endpoint = "search/tv" if m_type == "tv" else "search/movie"
##PY:            view_url_base = "tv" if m_type == "tv" else "movie"
##PY:            
##PY:            url_es = f"https://api.themoviedb.org/3/{search_endpoint}?api_key={TMDB_API_KEY}&query={requests.utils.quote(titulo_query)}&language=es-ES"
##PY:            url_en = f"https://api.themoviedb.org/3/{search_endpoint}?api_key={TMDB_API_KEY}&query={requests.utils.quote(titulo_query)}&language=en-US"
##PY:            
...
##PY:                    if results:
##PY:                        match = results[0]
##PY:                        titulo_real = match.get('name') if m_type == "tv" else match.get('title', titulo_query)
##PY:                        fecha = match.get('first_air_date') if m_type == "tv" else match.get('release_date', '')
...
##PY:            if INCLUDE_GITHUB: buttons_list.append({"label": "Savin-CinemaRPC", "url": GITHUB_URL})
...
##PY:                    os.system('cls')
##PY:                    print("=" * 75)
##PY:                    print(" 🎬   MPV DISCORD RICH PRESENCE - WINDOWS PREDICTIVE LAYOUT v3.3 🎬")
##PY:                    print("=" * 75)
...
##PY:                    print(f" 💻 Botón Web:       {'ACTIVADO' if INCLUDE_GITHUB else 'DESACTIVADO'}")
##PY:                    print("=" * 75)
##PY:                except: pass
##PY:        else:
